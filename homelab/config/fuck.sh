...
configure_sshd_motd() {
    ...
    # This line was already present and correctly handles the request for PrintLastLog no.
...
# create the .hushlogin file in the current user's home directory
create_hushlogin_file() {
    echo "Creating ~/.hushlogin to suppress 'Last login' message for the current user..."
    # The ~/.hushlogin file is created in the home directory of the user executing the script.
    # This complements the global PrintLastLog no setting in sshd_config.
    touch ~/.hushlogin
...
download_host_db
disable_dynamic_motd
configure_sshd_motd # This function already includes 'PrintLastLog no'
clear_static_motd
clear_profile_d_scripts
create_hushlogin_file # New function call to create ~/.hushlogin
create_custom_motd
